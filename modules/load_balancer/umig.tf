locals {
  ...
  backends = [for x in google_compute_instance_group.ftd : x.self_link]
}
...
resource "google_compute_instance_group" "ftd" {
  count   = var.num_instances
  name    = "ftd-${count.index + 1}-${random_string.suffix.result}"
  project = var.project_id
